...
def retry_rescan(logger):
    blk_dev_name=discover_new_lun(logger)
    if blk_dev_name:
        return blk_dev_name
    else:
        print('Rescanning...')
...
class VplxCrm(object):
    ...
    def _crm_verify(self, res_name, status):
        ...
        if verify_result:
            ...
            re_show_result = re_show.findall(verify_result['rst'].decode('utf-8'))
    ...
    def _crm_del(self, res_name):
        ...
        if del_result:
            ...
            re_result = re_delstr.findall(str(del_result['rst'], encoding='utf-8'))
            if len(re_result) == 2:
                return True
            else:
