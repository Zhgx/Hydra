...
class VplxDrbd(object):
    ...
    def prepare_config_file(self):
        ...
        for i in range(len(context)):
            ...
            if echo_result['sts']:
                ...
                s.pwe('Failed to prepare DRBD config file..', 3, 2)
    ...
    def _drbd_del_config(self, res_name):
        ...
        if del_result['sts']:
            ...
            s.pwe('Failed to remove DRBD config file', 4, 2)
...
class VplxCrm(object):
    ...
    def _setting_col(self):
        ...
        if result_crm:
            if result_crm['sts']:
                ...
                s.pwe(f'Failed to set colocation of "{self.lu_name}"', 4, 2)
        else:
            s.handle_exception()
    ...
    def _setting_order(self):
        ...
        if result_crm:
            if result_crm['sts']:
                ...
                s.pwe(f'Failed to set order of "{self.lu_name}"', 4, 2)
        else:
            s.handle_exception()
    ...
    def _crm_verify(self, res_name):
        ...
        if verify_result['sts'] == 1:
            ...
            s.pwe('Failed to show CRM resource', 4, 2)
    ...
    def _crm_stop(self, res_name):
        ...
        if crm_stop['sts']:
            if self.cyclic_check_crm_status(res_name, 'Stopped'):
                ...
                s.pwe('Failed to stop CRM resource ,exit the program...', 3, 2)
        else:
            s.pwe('Failed to stop CRM resource', 3, 2)
    ...
    def _crm_del(self, res_name):
        ...
        if del_result:
            ...
            if len(re_result) == 2:
                ...
                s.pwe(f'Failed to delete the iSCSILogicalUnit resource "{res_name}"', 3, 2)
