...
def find_session(logger):
    ...
    logger.write_to_log('T', 'INFO', 'info', 'start', oprt_id_one, '    Execute the command and check up the status of session')
    ...
    if result_session:
        ...
        if re_result:
            ...
            logger.write_to_log('T','INFO','warning','failed',oprt_id_one,'  ISCSI not login to VersaPLX, Try to login')
    ...
    def list_disk():
        ...
        if result_rescan:
            ...
            if result_lsscsi != None:
                ...
                logger.write_to_log('T', 'INFO', 'warning', 'failed', oprt_id, f'  Command "{cmd_lsscsi}" execute failed')
